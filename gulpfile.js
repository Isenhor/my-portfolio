...
gulp.task("ttf2woff2", async function () {
  gulp.src(["./src/fonts/*.ttf"]).pipe(ttf2woff2()).pipe(gulp.dest("build/fonts/"));
});
// Таск для компиляции SCSS в CSS
...
gulp.task("html:prettify", function () {
  return gulp.src("build/**/*.html").pipe(formatHtml()).pipe(gulp.dest("./build/"));
});
...
gulp.task(
  ...
  gulp.series(
    ...
    gulp.parallel("scss", "ttf2woff2", "pug", "copy:img", "copy:js", "copy:libs", "copy:video", "copy:php"),
